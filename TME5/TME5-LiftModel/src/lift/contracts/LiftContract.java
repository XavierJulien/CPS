...
public class LiftContract extends LiftDecorator {
	...
	public void checkInvariant() {
		...
		if(!(getLevel() >= getMinLevel())) {
			throw new InvariantError("L'étage courant n'est pas supérieur ou égal à l'étage minimum");
		}
		// inv:  getLevel() <= getMaxLevel()
		if(!(getLevel() <= getMaxLevel())) {
			throw new InvariantError("L'étage courant n'est pas inférieur ou égal à l'étage maximum");
		}
		...
		if(getLiftStatus()==LiftStatus.MOVING_UP || getLiftStatus()==LiftStatus.MOVING_DOWN) {
	...
	public void init(Integer minLevel, Integer maxLevel) {
		...
		if(!(getMinLevel() == minLevel)) {
			throw new PostconditionError("L'étage minimal est incorrectement initialisé");
		}
				
		// post: getMaxLevel() == maxLevel
		if(!(getMaxLevel() == maxLevel)) {
			throw new PostconditionError("L'étage maximal est incorrectement initialisé");
		}
		
		// post: getLevel() == minLevel
		if(!(getLevel() == minLevel)) {
			throw new PostconditionError("L'étage courant est incorrectement initialisé à l'étage minimal");
		}
		 
		// post: getLiftStatus() == IDLE
		if(!(getLiftStatus() == LiftStatus.IDLE)) {
			throw new PostconditionError("L'ascenseur n'est pas en attente à l'initialisation");
		}
		
		// post: getDoorStatus() == OPENED
		if(!(getDoorStatus() == DoorStatus.OPENED)) {
			throw new PostconditionError("La porte n'est pas ouverte à l'initialisation");
		}
		
		// post: getCommands().init()
		//       post: getCommands().getNbUpCommands() == 0
		if(!(getCommands().getNbUpCommands() == 0)) {
			throw new PostconditionError("Il y a des commandes de montée à l'initialisation");
		}
	...
	public void beginMoveUp() {
		...
		if(!(getDoorStatus() == DoorStatus.CLOSED)) {
			throw new PreconditionError("La porte n'est pas fermée avant un déplacement vers le haut");
		}
		...
		if(!(getLevel() < getCommands().getNextUpCommand())) {
			throw new PreconditionError("L'étage courant n'est pas strictement inférieur à la commande suivante");
		}
	...
	public void stepMoveUp() {
		// pre: getDoorStatus() == CLOSED
		if(!(getDoorStatus() == DoorStatus.CLOSED)) {
			throw new PreconditionError("La porte n'est pas fermée pendant un déplacement vers le haut");
		}
	...
	public void endMoveUp() {
		// pre: getDoorStatus() == CLOSED
		if(!(getDoorStatus() == DoorStatus.CLOSED)) {
			throw new PreconditionError("La porte n'est pas fermée en fin de déplacement vers le haut");
		}
	...
	public void beginMoveDown() {
		// pre: getDoorStatus() == CLOSED
		if(!(getDoorStatus() == DoorStatus.CLOSED)) {
			throw new PreconditionError("La porte n'est pas fermée avant un déplacement vers le bas");
		}
		// pre: getLiftStatus() == STANDBY_DOWN
		if(!(getLiftStatus() == LiftStatus.STANDBY_DOWN)) {
			throw new PreconditionError("L'ascenseur n'est pas prét en descente");
		}
		
		// pre: getLevel() > getCommands().getNextDownCommand()
		if(!(getLevel() > getCommands().getNextDownCommand())) {
			throw new PreconditionError("L'étage courant n'est pas strictement supérieur à la commande suivante");
		}
	...
	public void stepMoveDown() {
		// pre: getDoorStatus() == CLOSED
		if(!(getDoorStatus() == DoorStatus.CLOSED)) {
			throw new PreconditionError("La porte n'est pas fermée pendant un déplacement vers le bas");
		}
	...
	public void endMoveDown() {
		// pre: getDoorStatus() == CLOSED
		if(!(getDoorStatus() == DoorStatus.CLOSED)) {
			throw new PreconditionError("La porte n'est pas fermée en fin de déplacement vers le bas");
		}
		...
		if(!(getLiftStatus() == LiftStatus.STOP_DOWN)) {
			throw new PostconditionError("L'ascenseur ne stoppe pas en descente");
		}
	...
	public void openDoor() {
		...
		if(!(getLiftStatus() == LiftStatus.IDLE 
				|| getLiftStatus() == LiftStatus.STOP_UP
				|| getLiftStatus() == LiftStatus.STOP_DOWN)) {
			throw new PreconditionError("L'ascenseur n'est ni en attente, ni en arrêt");
		}
		...
		if(!(getDoorStatus() == DoorStatus.OPENING)) {
			throw new PostconditionError("La porte n'est pas en cours d'ouverture");
		}
	}
	...
	public void doorAck() {
		...
		if(!(getDoorStatus() == DoorStatus.OPENING
			|| getDoorStatus() == DoorStatus.CLOSING)) {
			throw new PreconditionError("La porte n'est ni en cours d'ouverture ni en cours de fermeture");
		}
		...
		if(getLiftStatus_atPre == LiftStatus.IDLE) {
			if(getCommands().getNbDownCommands() > 0) {
				if(!(getLiftStatus() == LiftStatus.STANDBY_DOWN)) {
					throw new PostconditionError("L'ascenseur n'est pas prêt en descente");
				}
			} else if(getCommands().getNbUpCommands() > 0) {
				if(!(getLiftStatus() == LiftStatus.STANDBY_UP)) {
	...
	public void selectLevel(int level) {
		...
		if(!(level >= getMinLevel())) {
			throw new PreconditionError("L'étage choisi est strictement inférieur é l'étage minimal");
		}
		// pre: level <= getMaxLevel()
		if(!(level <= getMaxLevel())) {
			throw new PreconditionError("L'étage choisi est strictement supérieur à l'étage maximal");
		}
		// pre: getLiftStatus() \in { IDLE, STANDBY_UP, STANDBY_DOWN }
		if(!(getLiftStatus() == LiftStatus.IDLE
			|| getLiftStatus() == LiftStatus.STANDBY_UP
			|| getLiftStatus() == LiftStatus.STANDBY_DOWN)) {
			throw new PreconditionError("L'ascenseur n'est ni en attente, ni prêt en montée ou en descente");
		}
