...
public class FilesPrioContract<T> extends FilesPrioDecorator<T> {
	...
	public FilesPrio<T> putPrio(int priorite, T elem) {
		...
		Set<Integer> getActivePrios_at_pre_without_prio = getActivePrios().remove(priorite);
		...
		// \post \forall k \from 2 \to getSizePrio(priorite)+1 {getElemPrio(priorite,k) == getElemPrio(priorite,k-1)@pre}
		for (int k : map_index_getElemPrio_at_pre.keySet()) {
			if (!getElemPrio(priorite, k).equals(map_index_getElemPrio_at_pre.get(k)))
				throw new PostcondError("Post failed : \\forall k \\from 2 \\to getSizePrio(priorite)+1 {getElemPrio(priorite,k) == getElemPrio(priorite,k-1)@pre}");
		}
		// \post \forall j \in getActivePrios \without {priorite} \forall k \from 1 \to getSizePrio(j)
		// {getElemPrio(j,k) == getElemPrio(j,k)@pre}
		for(int i : getActivePrios_at_pre_without_prio){
				for(int k : map_index_getElemPrio_at_pre.keySet()){
					if (!getElemPrio(i, k).equals(map_index_getElemPrio_at_pre.get(k)))
						throw new PostcondError("Post failed :  \post \forall j \in getActivePrios \without {priorite} \forall k \from 1 \to getSizePrio(j)
																										{getElemPrio(j,k) == getElemPrio(j,k)@pre}");
				}
		}
	...
	public FilesPrio<T> remove() {
		// pre-condition
		// \pre getSize() > 0
		// \post remove() =def= removePrio(getMaxPrio())
		if(getSize()<= 0) throw PrecondError("Pre failed : size <= 0");
		// pre-invariant
		checkInvariants();
		// capture
		//pas vraiment de capture vu que c'est fait dans removePrio
		// appel
		FilesPrio<T> res = super.remove();
		// post-invariant
		checkInvariants();
		// post-condition
		//pareil pas besoin car on fait appel directement a removePrio par definition
		return res;
	}
	...
	public FilesPrio<T> removePrio(int priorite) {
		// pre-condition
		// \pre getSizePrio(priorite) > 0
		if(getSizePrio(priorite) <= 0) throw PrecondError("Pre failed : size <= 0");
		// pre-invariant
		checkInvariants();
		// capture
		Set<Integer> getActivePrios_at_pre = getActivePrios();
		...
		// appel
		FilesPrio<T> res = super.removePrio(priorite);
		// post-invariant
		checkInvariants();
		// post-condition
		// \post getSizePrio(priorite) > 1 \imp (getActivePrios() == getActivePrios()@pre)
		if (getSizePrio(priorite)) > 1 && (!getActivePrios().equals(getActivePrios_at_pre)))
			throw new PostcondError("Post failed : getSizePrio(priorite) \\imp (getActivePrios() == getActivePrios()@pre)");
		// \post getSizePrio(priorite) == 1 \imp (getActivePrios() == getActivePrios()@pre) \without {priorite})
		if (getSizePrio(priorite)) == 1 && (!getActivePrios().equals(getActivePrios_at_pre_without_prio)))
			throw new PostcondError("Post failed : getSizePrio(priorite) \\imp (getActivePrios() == getActivePrios()@pre)");
		// \post getSizePrio(priorite) == getSizePrio(priorite)@pre -1
		if (getSizePrio(priorite) != (getSizePrio_at_pre-1))
			throw new PostcondError("Post failed : getSizePrio(priorite) == getSizePrio(priorite)@pre -1");
		// \post \forall j \in getActivePrios() \without {priorite} (getSizePrio(j) == getSizePrio(j)@pre)
		for (int j : map_index_getSizePrio_at_pre.keySet()) {
			if (!getSizePrio(priorite, k).equals(map_index_getSizePrio_at_pre.get(k)))
				throw new PostcondError("Post failed : \forall j \in getActivePrios() \without {priorite} (getSizePrio(j) == getSizePrio(j)@pre)");
		}
		// \post \forall k \from 1 \to getSizePrio(priorite)-1 (getElemPrio(priorite,k) == getElemPrio(priorite,k)@pre)
		for (int k : map_index_getElemPrio_at_pre.keySet()) {
			if (!getElemPrio(priorite, k).equals(map_index_getElemPrio_at_pre.get(k)))
				throw new PostcondError("Post failed : \\forall k \\from 1 \\to getSizePrio(priorite)-1 {getElemPrio(priorite,k) == getElemPrio(priorite,k)@pre}");
		}
		...
		for(int i : getActivePrios_at_pre_without_prio){
				for(int k : map_index_getElemPrio_at_pre.keySet()){
					if (!getElemPrio(i, k).equals(map_index_getElemPrio_at_pre.get(k)))
						throw new PostcondError("Post failed : \post \forall i \in getActivePrios() \without {priorite} \forall k \from 1 \to getSizePrio(j)
																									 {getElemPrio(i,k) == getElemePrio(i,k)@pre}");
				}
		}
		return res;
